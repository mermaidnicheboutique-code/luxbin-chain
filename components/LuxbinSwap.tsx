...
const SWAP_CONTRACT = "0x0000000000000000000000000000000000000000"; // Update with deployed swap contract

// For testnet, we'll use Base Sepolia
...
export function LuxbinSwap() {
  ...
  useEffect(() => {
    setIsWrongNetwork(chain?.id !== TARGET_CHAIN_ID);
  }, [chain]);
  ...
  const { data: rate } = useReadContract({
    ...
    abi: SWAP_ABI,
    functionName: "ethToLuxRate",
  });

  // Get contract balances
  const { data: luxBalance } = useReadContract({
    address: SWAP_CONTRACT as `0x${string}`,
    abi: SWAP_ABI,
    functionName: "getContractLuxBalance",
  });
      ...
      <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8">
        ...
        {isWrongNetwork && (
          <div className="mb-6 p-4 bg-red-500/10 border border-red-500/30 rounded-xl">
            <p className="text-sm text-red-400 mb-2">⚠️ Wrong Network</p>
            <p className="text-sm text-gray-300 mb-3">
              Please switch to Base Sepolia testnet to buy LUX tokens
            </p>
            <button
              onClick={() => switchChain({ chainId: TARGET_CHAIN_ID })}
              className="w-full bg-red-500/20 hover:bg-red-500/30 border border-red-500/50 text-red-300 py-2 px-4 rounded-lg transition-colors"
            >
        ...
        {!isWrongNetwork && (
          <div className="mb-6 p-4 bg-blue-500/10 border border-blue-500/30 rounded-xl">
            <p className="text-sm text-blue-400 mb-1">🧪 Testnet Mode</p>
            <p className="text-sm text-gray-300">
              Using Base Sepolia testnet. Get test ETH from faucet below.
            </p>
            <a
        ...
        {/* Contract Balances */}
        {(luxBalance !== undefined && ethBalance !== undefined) && (
          <div className="mb-6 p-4 bg-white/5 rounded-xl">
            <p className="text-sm text-gray-400 mb-2">Available Liquidity</p>
            <div className="flex justify-between text-sm">
              <span className="text-gray-300">
                {Number(formatEther(luxBalance)).toLocaleString()} LUX
              </span>
              <span className="text-gray-300">
